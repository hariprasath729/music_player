...
export default function SplashCursor({
  ...
  useEffect(() => {
    ...
    return () => {
      ...
      // WebGL resource cleanup to prevent GPU memory leaks
      try {
        if (gl) {
          const deleteFBO = (fboObj: any) => {
            if (!fboObj) return;
            if (fboObj.texture) gl.deleteTexture(fboObj.texture);
            if (fboObj.fbo) gl.deleteFramebuffer(fboObj.fbo);
          };
          const deleteDoubleFBO = (doubleFboObj: any) => {
            if (!doubleFboObj) return;
            deleteFBO(doubleFboObj.read);
            deleteFBO(doubleFboObj.write);
          };
        ...
        console.error('Error cleaning up WebGL in SplashCursor:', err);
      }
    };
  }, [
    SIM_RESOLUTION,
