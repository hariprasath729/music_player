...
export const getHomeData = async (req, res) => {
  try {
    ...
    const [likes, playlists, recentlyPlayed, playCounts] = await Promise.all([
      Like.find({ userId }),
    ...
    const recentIds = recentSongs.map(s => String(s.id));
    ...
    let madeForYou = songsCatalog.filter(s => 
      !baseIds.has(String(s.id)) && s.artist.split(',').some(a => preferredArtists.has(a.trim()))
    ).sort(() => 0.5 - Math.random()).slice(0, 20);

    // Fallback to general mix if user doesn't have enough history to base off yet
    if (madeForYou.length < 5) madeForYou = [...songsCatalog].filter(s => !baseIds.has(String(s.id))).sort(() => 0.5 - Math.random()).slice(0, 20);

    // 4. Top Artists
    const topArtistsMap = new Map();
    [...recentSongs, ...trending].forEach(song => {
      if (song) song.artist.split(',').forEach(a => topArtistsMap.set(a.trim(), (topArtistsMap.get(a.trim()) || 0) + 1));
    });
